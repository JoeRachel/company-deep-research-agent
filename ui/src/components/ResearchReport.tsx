...
const decodeMarkdownLinks = (markdown: string): string => {
  return markdown.replace(/\[(.*?)\]\((https?:\/\/[^\s\)]+)\)/g, (match, text, url) => {
    try {
      const decoded = decodeURIComponent(url);
      return `[${text || decoded}]( ${url} )`; // 保持跳转链接不变，展示标题或解码
    } catch {
      return match;
    }
  });
        ...
        <div className="mt-4">
          <ReactMarkdown
            ...
            components={{
                      ...
                      {parts.map((part, i) => 
                        urlRegex.test(part) ? (
                          <a 
                            key={i}
                            href={part}
                            className="text-[#468BFF] hover:text-[#8FBCFA] underline decoration-[#468BFF] hover:decoration-[#8FBCFA] cursor-pointer transition-colors break-words whitespace-normal"
                            target="_blank"
              ...
              li: ({node, ...props}) => (
                <li className="text-gray-800 break-words whitespace-normal" {...props} />
              ),
              a: ({node, href, ...props}) => (
                <a 
                  href={href}
          ...
          >
            {decodeMarkdownLinks(output.details.report || "No report available")}
