...
class BaseResearcher:
    def __init__(self):
        ...
        if not tavily_key or not siliconflow_key:
            raise ValueError("Missing API keys")
            
        self.tavily_client = AsyncTavilyClient(api_key=tavily_key)
        self.openai_client = AsyncOpenAI(api_key=siliconflow_key, base_url="https://api.siliconflow.cn/v1")
...
{self._format_query_prompt(prompt, company, hq, current_year)}"""
    ...
    def _format_query_prompt(self, prompt, company, hq, year):
        return f"""{prompt}
        重要指引：
        -仅聚焦于与{company}直接相关的信息
        -查询语句要非常简洁明了
        -提供恰好 4 条搜索查询语句，每行一条，不允许使用连字符或破折号
        -不得根据行业做任何假设，只能使用所提供的「industry」信息"""

    def _fallback_queries(self, company, year):
        return [
            f"{company} overview {year}",
            f"{company} Revenue Contribution Ratio {year}",
    ...
    async def search_single_query(self, query: str, websocket_manager=None, job_id=None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Add revenue topic for revenue analysts
            search_params = {
                "search_depth": "basic",
            ...
            if self.analyst_type == "revenue_analyst":
                search_params["topic"] = "revenue"
